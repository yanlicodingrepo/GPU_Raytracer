...
RGBA bilinearInterpTexel( const unsigned * texPixels,float x, float y, const int width, const int height, const int offset  )
 {
     if( x < 0 )
         x = 0.f;
     if( y < 0 )
...
Vector4 getReflectionDir( Vector3 norm, Vector4 incident )
{
    ...
    Vector4 reflection = (incident + Vector4( 2*cos*norm.x, 2*cos*norm.y, 2*cos*norm.z, 0 )).getNormalized();
    return reflection;
}
...
{
    const REAL n = n1/n2;
    const REAL cosI = -(norm.x*incident.x + norm.y*incident.y + norm.z*incident.z);
