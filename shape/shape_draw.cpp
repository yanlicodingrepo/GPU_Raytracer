...
void buildCubeVBO( int tess, GLuint &vbo, GLuint &vboIndex )
{
    ...
    for( int i = 0; i < cubeTess + 1; i++ )
    {
        for( int j = 0; j < cubeTess + 1; j++ )
        {
            ...
            vertices[index] = base + j*Vector3( unit,0,0 ) + i*Vector3( 0,0,unit );
            normals[index] = Vector3( 0.f, 1.f, 0.f );
            texCoords[index] = baseTex + j*Vector2( unit,0 ) + i*Vector2( 0,-unit );
            index++;
        }
    }
    ...
    for( int i = 0; i < cubeTess + 1; i++ )
    {
        for( int j = 0; j < cubeTess + 1; j++ )
        {
            vertices[index] = base + j*Vector3( 0, 0, unit ) + i*Vector3( 0, -unit, 0 );
    ...
    for( int i = 0; i < cubeTess + 1; i++ )
    {
        for( int j = 0; j < cubeTess + 1; j++ )
        {
            vertices[index] = base + j*Vector3( unit, 0, 0 ) + i*Vector3( 0, -unit, 0 );
    ...
    glBufferData( GL_ARRAY_BUFFER, (sizeVertices+sizeVertices)*sizeof(Vector3) + sizeVertices*sizeof(Vector2), NULL, GL_STATIC_DRAW );
    glBufferSubData( GL_ARRAY_BUFFER, 0, sizeVertices*sizeof(Vector3), vertices );
    glBufferSubData( GL_ARRAY_BUFFER, sizeVertices*sizeof(Vector3), sizeVertices*sizeof(Vector3), normals );
    glBufferSubData( GL_ARRAY_BUFFER, 2*sizeVertices*sizeof(Vector3), sizeVertices*sizeof(Vector2), texCoords );

    // Check the size
    int bufSize;
    glGetBufferParameteriv( GL_ARRAY_BUFFER, GL_BUFFER_SIZE_ARB, &bufSize );
    if( bufSize != sizeVertices*2*sizeof(Vector3) + sizeVertices*sizeof(Vector2) )
    {
        std::cerr<<"[buildCubeVBO] data size of vbo buffer is mismatch with input array\n";
        vbo = 0;
        vboIndex = 0;
    ...
    glBufferData( GL_ELEMENT_ARRAY_BUFFER, sizeof(GLuint)*sizeInd, indices, GL_STATIC_DRAW );

    glGetBufferParameteriv( GL_ELEMENT_ARRAY_BUFFER, GL_BUFFER_SIZE, &bufSize );

    if( bufSize != sizeInd*sizeof(GLuint) )
    {
        std::cerr<<"[buildCubeVBO] data size of element buffer is mismatch with input array\n";
        vbo = 0;
        vboIndex = 0;
...
void buildCylinderVBO( int tess, GLuint& vbo, GLuint& vboIndex )
{
    ...
    for( ; index < 3*(cylinderTess+1); index++ )
    {
        ...
        vertices[index].y = 0.5;
        texCoords[index] = Vector2( (2*M_PI-unitTheta*(index-base))/(2*M_PI), 1 );
        normals[index] = Vector3(cos(unitTheta*(index-base)), 0.f,sin(unitTheta*(index-base)) );
    }
    ...
    for( ; index < sizeVertices; index++ )
    {
        ...
        vertices[index].z = 0.5*sin(unitTheta*(index-base));
        vertices[index].y = -0.5;
        texCoords[index] = Vector2( (2*M_PI-unitTheta*(index-base))/(2*M_PI), 0 );
        normals[index] = Vector3(cos(unitTheta*(index-base)), 0.f,sin(unitTheta*(index-base)) );
    }
    ...
    {
        indices[index] = (i+1) >= 2*(cylinderTess+1)? cylinderTess+2 : (i+1);
        index++;
    }
   ...
   base = 2*(cylinderTess+1);
    ...
    if( bufSize != sizeVertices*2*sizeof(Vector3) + sizeVertices*sizeof(Vector2) )
    {
        std::cerr<<"[buildCylinderVBO] data size of vbo buffer is mismatch with input array\n";
        vbo = 0;
        vboIndex = 0;
        glBindBuffer( GL_ARRAY_BUFFER, 0 );
    ...
    if( bufSize != sizeInd*sizeof(GLuint) )
    {
        std::cerr<<"[buildCylinderVBO] data size of element buffer is mismatch with input array\n";
        vbo = 0;
        vboIndex = 0;
...
void buildConeVBO( int tess, GLuint& vbo, GLuint& vboIndex )
{
    ...
    for( int i = 0; i < coneTess+1; i++, index++ )
    {
        ...
        vertices[index] = Vector3( 0.f, 0.5f, 0.f );
        normals[index] = Vector3( normalH*cos(unitTheta*i), normalV,
    ...
    for( int i = 0; i < coneTess+1; i++, index++ )
    {
        vertices[index] = Vector3(0.5*cos(unitTheta*i), -0.5f, 0.5*sin(unitTheta*i) );
        normals[index] = Vector3( normalH*cos(unitTheta*i), normalV,
                                  normalH*sin(unitTheta*i)
                                  );
        texCoords[index] = Vector2( (2*M_PI-unitTheta*(i))/(2*M_PI), 0 );
    }
    ...
    for( int i = 0; i < coneTess; i++, index++ )
    {
        ...
        //vertices[index] = Vector3(0.f, 0.5f, 0.f );
        normals[index] = Vector3(0.f, -1.f, 0.f );
        texCoords[index] = Vector2( 0.5+0.5*cos(unitTheta*i), 0.5+0.5*sin(unitTheta*i) );
    }
    ...
    if( bufSize != sizeVertices*2*sizeof(Vector3)  + sizeVertices*sizeof(Vector2) )
    {
        std::cerr<<"[buildConeeVBO] data size of vbo buffer is mismatch with input array\n";
        vbo = 0;
        vboIndex = 0;
        glBindBuffer( GL_ARRAY_BUFFER, 0 );
    ...
    if( bufSize != sizeInd*sizeof(GLuint) )
    {
        std::cerr<<"[buildConeVBO] data size of element buffer is mismatch with input array\n";
        vbo = 0;
        vboIndex = 0;
...
void buildSphereVBO( int tess1, int tess2, GLuint &vbo, GLuint &vboIndex )
{
    ...
    for( int i = 0; i < sphereTess[0]+1; i++ )
    {
        for( int j = 0; j < sphereTess[1]+1; j++ )
        {
            ...
            texCoords[index] = Vector2( (2*M_PI-unitTheta*j)/(2*M_PI), (M_PI-unitPhi*i)/(M_PI));
            index++;
        }
    }
    assert( index == sizeVertices );
    ...
    glGetBufferParameteriv( GL_ARRAY_BUFFER, GL_BUFFER_SIZE_ARB, &bufSize );
    if( bufSize != (sizeVertices+sizeVertices)*sizeof(Vector3) + sizeVertices*sizeof(Vector2) )
    {
        std::cerr<<"[buildSphereVBO] data size of vbo buffer is mismatch with input array\n";
        vbo = 0;
        vboIndex = 0;
        glBindBuffer( GL_ARRAY_BUFFER, 0 );
    ...
    if( bufSize != sizeInd*sizeof(GLuint) )
    {
        std::cerr<<"[buildSphereVBO] data size of element buffer is mismatch with input array\n";
        vbo = 0;
        vboIndex = 0;
...
void drawCube( GLuint vbo, GLuint vboElement, GLuint texHandle )
{
    ...
    glFrontFace( GL_CW );
    glDrawElements( GL_TRIANGLE_STRIP, indicesArrayLengthEach, GL_UNSIGNED_INT, (GLuint*)NULL + indicesArrayLengthEach );
    glFrontFace( GL_CCW );
    glDrawElements( GL_TRIANGLE_STRIP, indicesArrayLengthEach, GL_UNSIGNED_INT, (GLuint*)NULL + indicesArrayLengthEach*2 );
    glFrontFace( GL_CW );
    glDrawElements( GL_TRIANGLE_STRIP, indicesArrayLengthEach, GL_UNSIGNED_INT, (GLuint*)NULL + indicesArrayLengthEach*3 );
    glFrontFace( GL_CCW );
    glDrawElements( GL_TRIANGLE_STRIP, indicesArrayLengthEach, GL_UNSIGNED_INT, (GLuint*)NULL + indicesArrayLengthEach*4 );
    glFrontFace( GL_CW );
    glDrawElements( GL_TRIANGLE_STRIP, indicesArrayLengthEach, GL_UNSIGNED_INT, (GLuint*)NULL + indicesArrayLengthEach*5 );
...
void drawCylinder( GLuint vbo, GLuint vboElement, GLuint texHandle )
{
    ...
    glFrontFace( GL_CW );
    glDrawElements( GL_TRIANGLE_FAN, indicesArrayLengthTopBot, GL_UNSIGNED_INT, (GLuint*)0 );
    glFrontFace( GL_CCW );
    glDrawElements( GL_TRIANGLE_FAN, indicesArrayLengthTopBot, GL_UNSIGNED_INT, (GLuint*)0 + indicesArrayLengthTopBot);
    glFrontFace( GL_CW );
    glDrawElements( GL_TRIANGLE_STRIP, indicesArrayLengthEach, GL_UNSIGNED_INT, (GLuint*)0 + indicesArrayLengthTopBot*2 );
...
void drawSphere( GLuint vbo, GLuint vboElement, GLuint texHandle )
{
    ...
    glFrontFace( GL_CW );
    glDrawElements( GL_TRIANGLES, (sphereTess[1])*(sphereTess[0])*3*2, GL_UNSIGNED_INT, (GLuint*)0  );
