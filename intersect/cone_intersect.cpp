                          ...
                          )
{
    ...
    REAL B = 2*eyePos.x*d.x + 2*eyePos.z*d.z - 0.5*eyePos.y*d.y + 0.25*d.y;
    REAL C = eyePos.x*eyePos.x + eyePos.z*eyePos.z - 0.25*eyePos.y*eyePos.y + 0.25*eyePos.y - 0.0625;
        ...
        )
{
     Vector3 norms[2];
      norms[0] = Vector3(0,-1,0);
     norms[1] = Vector3(2*(intersect.x), 0.25 - 0.5*(intersect.y), 2*(intersect.z));
