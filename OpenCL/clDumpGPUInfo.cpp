...
void dumpGPUInfo()
{
    ...
    for (int p=0; p < platforms.size(); p++)
    {
        ...
        cout << "CL DUMP: platform = " << p << " name = " << platform_name.toStdString() << endl;

        //devices
        QVector<cl_device_id> devices = getDeviceList(platforms.at(p), CL_DEVICE_TYPE_GPU);

         cout<< "CL DUMP: platform = " << p << " device_count = " << devices.size() << endl;
        for (int d=0; d < devices.size(); d++)
        {
            QString device_name = getDeviceName(devices[d]);
            cout << "CL DUMP: platform = " << p << " device = " << d << " name = " << device_name.toStdString() << endl;
        }
    }
}
