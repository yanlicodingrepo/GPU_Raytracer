...
SceneObject::~SceneObject()
{
    ...
    /*if( m_primitive.material.textureMap )
        delete m_primitive.material.textureMap;
    if ( m_primitive.material.bumpMap )
        delete m_primitive.material.bumpMap;
    */
}
...
void Scene::recursiveParseNode( Scene *sceneToFill, CS123SceneNode *node, Matrix4x4 transform)
{
...
void Scene::drawPrimitive( PrimitiveType type, const VboHandles* vbos, GLuint texHandle )
{
    switch( type )
    {
    case PRIMITIVE_CUBE:
...
void Scene::renderGeometry(bool useMaterials, const VboHandles* vbos )
{
    for( int i = 0; i < m_objects.size(); i++ )
    {
...
void Scene::addPrimitive(const CS123ScenePrimitive &scenePrimitive, const Matrix4x4 matrix)
{
    ...
    {
    case PRIMITIVE_CUBE:obj.m_boundingBox = computeCubeAABB(obj.m_transform);break;
    case PRIMITIVE_CONE:obj.m_boundingBox = computeConeAABB(obj.m_transform);break;
    case PRIMITIVE_CYLINDER:obj.m_boundingBox = computeCylinderAABB(obj.m_transform);break;
    case PRIMITIVE_SPHERE:obj.m_boundingBox = computeSphereAABB(obj.m_transform);break;
    case PRIMITIVE_TORUS:
    case PRIMITIVE_MESH:
    default:
        assert(0);
 ...
 if( bakPos == Vector3( POS_INF, POS_INF, POS_INF) )
    ...
    if( path.size() && scenePrimitive.material.textureMap->isUsed )
    {
        ...
        {
            obj.m_texture.m_textureHandle = loadTexture( path, obj.m_primitive.material.blend );
