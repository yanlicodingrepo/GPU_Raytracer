...
void GPURayScene::render()
{
    ...
    cl_float clEyeNear = m_camera->getNear();
     ...
     ciErrNum |= clSetKernelArg(m_cl->m_kernelRay, 10, sizeof(cl_float), (void*)&clEyeNear );
     ciErrNum |= clSetKernelArg(m_cl->m_kernelRay, 11, sizeof(cl_float16), (void*)&clInvMat);
      ...
      cl_mem canvas_dev = clCreateFromGLTexture2D(m_cl->m_context, CL_MEM_WRITE_ONLY, GL_TEXTURE_2D, 0, m_objects[0].texHandle, &ciErrNum);
      cl_image_format format;
      int a = CL_UNORM_INT8;
      clGetImageInfo( canvas_dev, CL_IMAGE_FORMAT,sizeof(cl_image_format),&format, NULL );
...
void GPURayScene::pushSceneData( Scene* scene )
{
    ...
    for( int i = 0; i < objects.size(); i++ )
    {
        ...
        newObj.invTransform = copySceneMatrix( curObj.m_invTransform );
        newObj.invTTransformWithoutTrans = copySceneMatrix( curObj.m_invTTransformWithoutTrans );
    ...
    copyKdTree(scene,  scene->getKdTree() );
    ...
    for(int i = 0 ; iter != texMap.end(); iter++, i++ )
    {
        memcpy(m_pixels+m_textureOffsets[i], (*iter).m_texPointer,
...
void GPURayScene::setKernelArgs()
{
    ...
    // Reset argument, necessary?
       ...
       ciErrNum |= clSetKernelArg(m_cl->m_kernelRay, 12, sizeof(cl_mem), (void*)&m_cmTexPixelBuffer );
        ciErrNum |= clSetKernelArg(m_cl->m_kernelRay, 13,sizeof(cl_int), (void*)&m_pixelNum );
        ciErrNum |= clSetKernelArg(m_cl->m_kernelRay, 14, sizeof(cl_mem), (void*)&m_cmOffsetBuffer);
        ciErrNum |= clSetKernelArg(m_cl->m_kernelRay, 15, sizeof(cl_uint), (void*)&offsetCount );
        ciErrNum |= clSetKernelArg(m_cl->m_kernelRay, 16, sizeof(cl_mem), (void*)&m_cmKdNodes);
        ciErrNum |= clSetKernelArg(m_cl->m_kernelRay, 17, sizeof(cl_uint), (void*)&kdnodeCount );
        ciErrNum |= clSetKernelArg(m_cl->m_kernelRay, 18, sizeof(cl_mem), (void*)&m_cmObjNodes);
        ciErrNum |= clSetKernelArg(m_cl->m_kernelRay, 19, sizeof(cl_uint), (void*)&objnodeCount );
...
void GPURayScene::copyKdTree( Scene* scene, KdTree* tree )
{
    ...
    for( int i = 0; i < KDTREE_ARRAY_SIZE; i++ )
    {
        ...
        m_kdNodes[i].boxBegin = copySceneVector3(kdNodeArray[i].getAABB().getPos());
        m_kdNodes[i].boxSize = copySceneVector3(kdNodeArray[i].getAABB().getSize());
...
void GPURayScene::dumpCLKdTree( std::string fileName )
{
    ...
    out<<"Extends: "<<endl<<(float)root.boxBegin.x<<" "<<(float)root.boxBegin.y<<" "
      <<(float)root.boxBegin.z<<endl;
    out<<(float)( root.boxBegin.x + root.boxSize.x )<<" "
      <<(float)( root.boxBegin.y + root.boxSize.y )<<" "
        <<(float)( root.boxBegin.z + root.boxSize.z )<<endl;

    dumpKdTreeInfoLeaf( 0, out, 0 );
}

void GPURayScene::dumpKdTreeInfoLeaf( int curIndex, std::ofstream& ofile, int depth )
{
    ...
    if( cur.leftIndex != -1 )
    {
        dumpKdTreeInfoLeaf( cur.leftIndex, ofile, depth+1 );
    }
    if( cur.rightIndex != -1 )
    {
        dumpKdTreeInfoLeaf( cur.rightIndex, ofile, depth+1);
    }
}
...
{
   // int a = CL_DEVICE_MAX_MEM_ALLOC_SIZE;
    ...
    if( m_pixels )
    {
        ...
        //TODO: TEST IF DELETING m_pixels causes error
    // Create the buffer for texture pixels

    if( ciErrNum1 | ciErrNum2 |  ciErrNum3| ciErrNum4 | ciErrNum5 | ciErrNum6 != CL_SUCCESS )
    {
